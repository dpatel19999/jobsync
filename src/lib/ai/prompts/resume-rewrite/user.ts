/**
 * Position-Locked Resume Rewrite — User Prompt (EN)
 */
...
export function buildResumeRewritePrompt(
  ...
  return `Rewrite the ORIGINAL RESUME below, line by line, to better match the TARGET JOB DESCRIPTION's language and keywords — without adding, removing, merging, or reordering any line.
...
Output only the rewritten resume text, with EXACTLY ${lineCount} non-empty lines, in the same order as the original resume above.`;
}
